...
function NightMap({ merchants }) {
  ...
  return (
    <MapContainer center={center} zoom={5} style={{ height: '100%', width: '100%', background: '#000' }} zoomControl={false}>
        ...
        <Marker key={m.id} position={m.coords} icon={goldIcon}>
          <Popup>
            <div style={{ background: '#111', color: '#fff', padding: '12px', borderRadius: '12px', minWidth: '160px' }}>
              ...
              {m.city && <p style={{ color: '#666', fontSize: '10px', marginTop: '4px' }}>📍 {m.city}</p>}
            </div>
          </Popup>
        </Marker>
      ))}
...
export default function HomePage({ onEnterStore, onGoToAuth }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (error || !data) setSearchError('Aucune boutique trouvée avec ce code.');
      else setSearchResult(data);
    } catch { setSearchError('Erreur de recherche. Réessayez.'); }
    finally { setIsSearching(false); }
  };

  const activeCount = merchants.filter(m => m.coords).length;
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-20 min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-xl text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#D4AF37] mb-4">
              {activeCount} agence{activeCount > 1 ? 's' : ''} active{activeCount > 1 ? 's' : ''} sur le réseau
            </p>
            ...
            <form onSubmit={handleSearch}>
              <div className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input type="text" placeholder="Ex: ZKARS" value={searchCode}
                ...
                <button type="submit" disabled={isSearching || !searchCode}
                  className="px-8 py-5 bg-[#D4AF37] text-black rounded-2xl font-black uppercase text-[10px] tracking-widest hover:scale-105 transition-all disabled:opacity-50">
                  {isSearching
              ...
              {searchResult && (
                <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                  ...
                  <button onClick={() => onEnterStore(searchResult)}
                    className="w-full py-4 bg-[#D4AF37] text-black rounded-xl font-black uppercase text-[10px] tracking-widest hover:scale-[1.02] transition-all flex items-center justify-center gap-3">
        ...
        {/* Footer — neutre, sans suggestions d'agences */}
        <footer className="p-6 md:p-10 flex items-center justify-center">
          <p className="text-zinc-700 text-[9px] font-bold uppercase tracking-widest">
            Shift Luxury · Réseau mondial de location prestige
          </p>
        </footer>
      </div>
    </div>
